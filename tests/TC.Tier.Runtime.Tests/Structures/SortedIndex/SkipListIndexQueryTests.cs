...
/// <summary>
/// SkipListIndex 位置化查询契约测试（SeekLowerBound / TryGetMax / TryGetFloor——SkipListIndex.Query.cs +
/// SkipListIndex.Cursor.cs 定位化）。
/// <para>★ 契约与 BTreeIndexQueryTests 同一矩阵（比较族公共语义）：Seek 后 Current 即首个 ≥ key 的
///   条目、MoveNext 产出后续；Max/Floor 为键序 Latest/采样语义（含等值）。删除物理拆链无空节点。</para>
/// </summary>
public class SkipListIndexQueryTests : IDisposable
{
    readonly TestVolume _vol;
    bool _disposed;

    public SkipListIndexQueryTests()
    {
        _vol = new TestVolume();
    }
    ...
    static SkipListIndex<long> CreateSkipList(TestVolume vol, string name = "sl")
    {
    ...
    {
        using var index = CreateSkipList(_vol);
        index.Invoking(i => i.CreateScanCursor(ReadDirection.Backward))
            .Should().Throw<NotSupportedException>(
                "层 0 链单向——Backward 显式抛优于谎报（旧形接受参数恒前向产出）");
    ...
    public void Seek_EmptyTree_ReturnsFalse()
    {
        ...
        cursor.MoveNext().Should().BeFalse("空表 seek 后游标置于末尾");
    }
    ...
    public void Seek_BeforeFirst_PositionsAtFirstEntry()
    {
        ...
        cursor.CurrentKey.Should().Be(10, "Seek 后 Current 即首个 ≥ key 条目（与 BTree 同契约）");
        cursor.MoveNext().Should().BeTrue();
        cursor.CurrentKey.Should().Be(20, "MoveNext 交付定位条目的后续");
    }

    [Fact]
    public void Seek_ExactKey_DeliversKeyFirst()
    {
        using var index = CreateSkipList(_vol);
        foreach (var k in new long[] { 10, 20, 30 })
            index.Insert(k, MakeAddr(k), LogicalAddress.Empty);
    ...
    [Fact]
    public void Seek_BetweenKeys_DeliversNextGreater()
    {
        using var index = CreateSkipList(_vol);
        foreach (var k in new long[] { 10, 20, 30 })
            index.Insert(k, MakeAddr(k), LogicalAddress.Empty);
    ...
    [Fact]
    public void Seek_MultiNode_DeliversSortedSuffix()
    {
        using var index = CreateSkipList(_vol);
        ...
        var delivered = new List<long> { cursor.CurrentKey };
    ...
    [Fact]
    public void Max_EmptyTable_ReturnsFalse()
    {
        using var index = CreateSkipList(_vol);
    ...
    [Fact]
    public void Floor_EmptyTable_ReturnsFalse()
    {
        using var index = CreateSkipList(_vol);
    ...
    // ══════════════════════════════════════════════════════════
    // 随机属性测试（对照有序表 oracle——多塔形态下的全路径覆盖）
    // ══════════════════════════════════════════════════════════
