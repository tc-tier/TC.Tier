...
public class NodeKeyPairTests : IDisposable
{
    ...
    /// <summary>★解压高位零字节回归（flaky 根因判例）：大量随机密钥对的 compressed→解压→
    ///   验签往返——y 值高位字节为零（~1/256/密钥对）曾因 stackalloc 垃圾残留致公钥点静默错误。
    ///   500 对 × 双操作 = 覆盖概率 ~86% 至少触发一次高位零 y；确定性错在修复前必现失败。</summary>
    [Fact]
    public void CompressedDecompress_HighZeroByteY_VerifyAlwaysSucceeds()
    {
        ...
        for (var i = 0; i < 500; i++)
        {
            ...
            NodeKeyPair.Verify(key.PublicKey, data, signature).Should().BeTrue($"第 {i} 对——解压公钥点必须精确（高位零 y 不容垃圾残留）");
        }
    }
