...
/// <summary>
/// AsyncPriorityQueueV3（Route C 依赖追踪式回收版）契约测试——与 AsyncPriorityQueueTests（Route A）、
/// AsyncPriorityQueueV2Tests（B' 绊线）同款测试族，外加分配对比测试（V3 零分配）。
/// <para>★ 压力测试即 Route C 协议验证器：V2 基线约 50% 轮次命中绊线（schedule 重复/陈旧引用），
/// V3 要求 0 fail——count==0 回收判据 + gen-in-flags 使该竞态状态不可表示。</para>
/// </summary>
public class AsyncPriorityQueueV3Tests
{
    ...
    {
        var q = new AsyncPriorityQueueV3<int>(CreateEpoch());
    ...
    {
        var q = new AsyncPriorityQueueV3<int>(CreateEpoch(), capacity: 16384);
        const int n = 10000;
    ...
    {
        var q = new AsyncPriorityQueueV3<int>(CreateEpoch(), capacity: 256);
    ...
    {
        var q = new AsyncPriorityQueueV3<string>(CreateEpoch());
    ...
    {
        var q = new AsyncPriorityQueueV3<int>(CreateEpoch(), capacity: 4096);
    ...
    {
        // ★ Route C 协议验证器：重型并发下 count 判据 + gen-in-flags 必须保持回收协议完整
        //   （V2 基线约 50% 轮次在此命中"schedule 重复/陈旧引用"绊线；V3 目标 0 fail）
        var q = new AsyncPriorityQueueV3<int>(CreateEpoch(), capacity: 16384);
    ...
    {
        // ★ Route C 协议验证器（V2 同款：该测试是"一代陈旧引用"的最灵敏探测器）
        var q = new AsyncPriorityQueueV3<int>(CreateEpoch(), capacity: 4096);
    ...
    // ═══════════════════════════════════════════════════════════
    // ★ Route C 核心验证：热路径零托管分配（对比 A 每次入队一次 Gen0）
    // ═══════════════════════════════════════════════════════════

    [Fact(Skip = "实验版本 V2/V3——非生产，默认跳过（RootCause 见 docs/lab/async-priority-queue-root-cause.md）")]
    public void Allocation_V3_EnqueueDequeueCycle_ZeroAllocations()
    {
        var q = new AsyncPriorityQueueV3<int>(CreateEpoch(), capacity: 1024);
        ...
        allocated.Should().Be(0, $"Route C（槽位池化 + 计数回收）热路径应零托管分配，实际 {allocated} 字节");
        q.ValidateInvariants();
    }
    ...
    {
        Action act = static () => _ = new AsyncPriorityQueueV3<int>(null!);
