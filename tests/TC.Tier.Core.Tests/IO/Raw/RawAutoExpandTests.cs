...
/// </summary>
public sealed class RawAutoExpandTests : IDisposable
{
    private readonly string _dir = TestTempDir.Create("core-io-raw-autoexpand");
    private readonly List<RawFileSystem> _openFs = [];

    private string NewVolumePath() => Path.Combine(_dir, $"vol-{Guid.NewGuid():N}.raw");

    private RawFileSystem NewAutoVolume()
    {
        var fs = RawFileSystem.New(RawCarrier.File(NewVolumePath()), new RawFormatOptions());   // QuotaBytes 缺省 -1
        _openFs.Add(fs);
        return fs;
    }
    ...
    public void New_WithoutQuota_SurvivesCleanCloseAndReopen()
    {
        ...
        using (var fs = RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions { Label = "auto" }))
        {
            fs.CreateFile("seed", preallocateSize: 4096);
        }
        using var reopened = Reopen(path, new RawOpenOptions { Label = "auto" });
    ...
    public void SpaceExhaustion_GrowsAndDataSurvivesReopen()
    {
        ...
        using (var fs = RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions()))
        {
            fs.CreateFile("big");
    ...
    public void CrashAfterGrowth_JournalRecoversDataAndCapacity()
    {
        ...
        var fs = RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions());
    ...
    private RawFileSystem ReopenAfterNew(string path, long quotaBytes)
    {
        using (RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions())) { }
        return Reopen(path, new RawOpenOptions { QuotaBytes = quotaBytes });
    }
    ...
    public void OpenQuota_CapsGrowthAtQuota()
    {
        ...
        using var fs = Reopen(path, new RawOpenOptions { QuotaBytes = 72L << 20 });
        var files = 0;
        while (true)
        {
            try
    ...
    {
        using var fs = NewAutoVolume();
        fs.AddCarrier(RawCarrier.File(Path.Combine(_dir, $"m1-{Guid.NewGuid():N}.raw")), capacityBytes: 16L << 20);
    ...
    {
        var vol = Path.Combine(_dir, "spec-grow.raw");
        var spec = vol.Replace('\\', '/');
        using (var fs = (RawFileSystem)TierFs.New($"virtual:///{spec}?label=grow"))
        {
        ...
        using (var reopened = (RawFileSystem)TierFs.Open($"virtual:///{spec}"))
        {
    ...
    {
        var vol = Path.Combine(_dir, "spec-multi.raw");
        var spec = vol.Replace('\\', '/');
        var ex = Assert.Throws<NotSupportedException>(
            () => TierFs.New($"virtual:///{spec}?member=/nonexistent/v2.raw"));
