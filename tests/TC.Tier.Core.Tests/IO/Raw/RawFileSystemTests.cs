...
/// <summary>
/// RawFileSystem 契约测试（raw-medium-and-conversion-design §2/§3/§4）——
/// 格式与打开 / 一卷一实例 / 双 superblock / 未知保留值拒开 / 断电恢复（dirty → 可写继续）/
/// 命名空间 CRUD / 能力位 / 卷几何精确性。
/// </summary>
public sealed class RawFileSystemTests : IDisposable
{
    private readonly string _dir = TestTempDir.Create("core-io-raw");
    private readonly List<RawFileSystem> _openFs = [];

    private string NewVolumePath() => Path.Combine(_dir, $"vol-{Guid.NewGuid():N}.raw");

    private RawFileSystem Format(long capacity = 16L << 20, RawFormatOptions? options = null)
    {
    ...
    private RawFileSystem Reopen(string path, RawOpenOptions? options = null)
    {
        var fs = RawFileSystem.Open(RawCarrier.File(path), options);
        _openFs.Add(fs);
        return fs;
    }
    ...
    public void Format_ThenOpen_Succeeds()
    {
        ...
        using (var fs = RawFileSystem.New(RawCarrier.File(path),
                   new RawFormatOptions { QuotaBytes = 8 << 20, Label = "test-vol" }))
        {
            fs.VolumeUuid.Should().NotBe(Guid.Empty);
            fs.Volume.TotalSpace.Should().Be(8 << 20, "TotalSpace 精确（§3.5 增强行）");
        }
        using var reopened = RawFileSystem.Open(RawCarrier.File(path));
    ...
    {
        var path = NewVolumePath();
        using (var fs = RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions { QuotaBytes = 8 << 20 })) { }
        var act = () => RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions { QuotaBytes = 8 << 20 });
    ...
    {
        var path = NewVolumePath();
        using (RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions { QuotaBytes = 8 << 20 })) { }
        // 模拟 v2+ 卷：未知 flags + 重算的合法 CRC（单纯篡改会先被 CRC 拦——那是另一条正确防线）
        // 0x0010 = 未认领位（0x0002 Journaled / 0x0004 MultiCarrier / 0x0008 AutoExpand 已认领——RM-03/RM-04/§5.3）
        foreach (var sbOffset in new[] { 0L, 4096L })
        {
            ...
            System.Buffers.Binary.BinaryPrimitives.WriteUInt16LittleEndian(sb.AsSpan(6), 0x0010);
    ...
    public void CrashRecovery_DirtyReopen_WritableContinues()
    {
        ...
        var fs1 = RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions { QuotaBytes = 16 << 20 });
        ...
        using var fs2 = RawFileSystem.Open(RawCarrier.File(path));
    ...
    {
        var path = NewVolumePath();
        using (var fs = RawFileSystem.New(RawCarrier.File(path), new RawFormatOptions { QuotaBytes = 16 << 20 }))
        {
