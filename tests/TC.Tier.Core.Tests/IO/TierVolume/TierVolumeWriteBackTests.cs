...
/// </summary>
public sealed class TierVolumeWriteBackTests : IDisposable
{
    private readonly string _dir = TestTempDir.Create("core-io-tv-wb");
    private readonly string _volPath;

    public TierVolumeWriteBackTests() => _volPath = Path.Combine(_dir, "v.tier");

    public void Dispose() => TestTempDir.TryCleanup(_dir);

    private TierVolumeFs Format()
        => TierVolumeFs.New(TierVolumeCarrier.File(_volPath), new TierVolumeFormatOptions { QuotaBytes = 32L << 20 });
    ...
    // ═══ RM-40：非日志卷持久化语义（写绕/数据-only 脏的 Flush 必须屏障）═══

    private TierVolumeFs FormatNoJournal()
        => TierVolumeFs.New(TierVolumeCarrier.File(_volPath), new TierVolumeFormatOptions
        {
            QuotaBytes = 32L << 20,
            JournalReserveBytes = 0,   // 非日志卷（检查点模式）
        });

    [Fact]
    public void NoJournal_OverwriteFlush_BarrierCoversWriteAround_RM40()
    {
        using var fs = FormatNoJournal();
        using var h = fs.Open("f", RWO());
        var block = new byte[4 * 4096];
        new Random(13).NextBytes(block);
        h.Write(0, block);
        h.Flush();   // 结构脏 → 检查点（数据随屏障 #1 落盘）
        fs.CarrierWritePendingBytes.Should().Be(0, "检查点屏障后在途归零");

        new Random(14).NextBytes(block);
        h.Write(0, block);   // 覆写——结构净：写绕直落（不标脏、无记录）
        fs.CarrierWritePendingBytes.Should().Be(block.Length, "覆写直落字节入在途计数");
        h.Flush();   // 非日志 Flush：MetadataDirty false → FlushDirtyPages（屏障）
        fs.CarrierWritePendingBytes.Should().Be(0, "非日志 Flush 屏障覆盖写绕数据（fsync 语义）");
    }

    [Fact]
    public void NoJournal_OverwriteFlushRoot_BarrierCoversWriteAround_RM40()
    {
        using var fs = FormatNoJournal();
        using var h = fs.Open("f", RWO());
        var block = new byte[4 * 4096];
        new Random(15).NextBytes(block);
        h.Write(0, block);
        h.Flush();
        new Random(16).NextBytes(block);
        h.Write(0, block);   // 覆写（写绕在途）
        fs.CarrierWritePendingBytes.Should().Be(block.Length);
        fs.FlushRoot();   // sync() 语义——数据-only 脏也须屏障
        fs.CarrierWritePendingBytes.Should().Be(0, "FlushRoot 覆盖写绕数据");
    }

    [Fact]
    public void NoJournal_DirectOverwriteFlush_BarrierCoversDirectCarrierWrites_RM40()
    {
        using var fs = FormatNoJournal();
        using var h = fs.Open("f", new FileOpenOptions
        {
            Access = AccessMode.ReadWrite, Mode = FileOpenMode.OpenOrCreate,
            Sharing = FileSharing.ReadWrite, Hints = FileOpenHints.NoBuffering,   // 直达档——绕过自管页缓存
        });
        var block = new byte[4 * 4096];
        new Random(17).NextBytes(block);
        h.Write(0, block);
        h.Flush();
        new Random(18).NextBytes(block);
        h.Write(0, block);   // 直达档覆写直落载体——同样须计入在途
        fs.CarrierWritePendingBytes.Should().Be(block.Length, "直达档直落字节入在途计数");
        h.Flush();
        fs.CarrierWritePendingBytes.Should().Be(0, "直达档 Flush 屏障覆盖直落数据");
    }
}