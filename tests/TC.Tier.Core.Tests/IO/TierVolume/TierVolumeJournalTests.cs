...
/// </summary>
public sealed class TierVolumeJournalTests : IDisposable
{
    private readonly string _dir = TestTempDir.Create("core-io-tv-journal");
    private readonly string _volPath;

    public TierVolumeJournalTests() => _volPath = Path.Combine(_dir, "v.tier");

    public void Dispose() => TestTempDir.TryCleanup(_dir);

    private TierVolumeFs Format(long journalBytes = 8L << 20, long capacity = 64L << 20)
    ...
    public void ReplayDeterministic_SecondRecoveryIdempotent()
    {
        ...
        using var fs3 = TierVolumeFs.Open(TierVolumeCarrier.File(_volPath));
    ...
    public void JournalFieldsWithoutFlag_Refused_OldVolumeOpens()
    {
        ...
        var path = Path.Combine(_dir, "oldstyle.tier");
        ...
        // 老卷（无日志）在检查点模式照常工作
        var path2 = Path.Combine(_dir, "plain.tier");
        var fs = TierVolumeFs.New(TierVolumeCarrier.File(path2), new TierVolumeFormatOptions
        ...
        fs.Dispose();
        using var fs2 = TierVolumeFs.Open(TierVolumeCarrier.File(path2));
    ...
    // ═══ RM-40 持久化语义（写绕 + 空转快道绕过 fsync——Flush 返回时数据必须已落盘）═══

    [Fact]
    public void OverwriteFlush_BarrierCoversWriteAroundBytes_RM40()
    {
        var fs = Format();
        using var h = fs.Open("f", RWO());
        var block = new byte[4 * 4096];
        new Random(7).NextBytes(block);
        h.Write(0, block);   // 首写（整块 run 写绕直落——新文件：ExtentAppend 记录）
        h.Flush();
        fs.CarrierWritePendingBytes.Should().Be(0, "首写提交屏障后在途载体写归零");

        new Random(8).NextBytes(block);
        h.Write(0, block);   // 覆写：写绕直落载体——不标脏、不发记录（旧实现 Flush 空转零 fsync）
        fs.CarrierWritePendingBytes.Should().Be(block.Length, "写绕直落字节入在途计数（脏页 ∪ 在途 = 数据-only 判据）");
        h.Flush();
        fs.CarrierWritePendingBytes.Should().Be(0, "Flush 屏障覆盖写绕数据——fsync 语义（旧实现 0.011ms 空转 vs 裸 fsync 17-21ms）");
    }

    [Fact]
    public void FlushData_BarrierCoversWriteAroundBytes_RM40()
    {
        var fs = Format();
        using var h = fs.Open("f", RWO());
        var block = new byte[4 * 4096];
        ...
        h.FlushData();   // fdatasync 形态——数据面屏障（不含记录提交）
        fs.CarrierWritePendingBytes.Should().Be(0, "FlushData 屏障覆盖写绕数据");
    }

    [Fact]
    public void WriteThrough_Overwrite_PerWriteBarrierCoversWriteAround_RM40()
    {
        var fs = Format();
        ...
        h.Write(0, block);
        fs.CarrierWritePendingBytes.Should().Be(0, "写透首写（记录提交）后在途归零");
        new Random(12).NextBytes(block);
        h.Write(0, block);   // 覆写——无记录：逐写提交必须仍补屏障（O_SYNC 语义）
        fs.CarrierWritePendingBytes.Should().Be(0, "写透覆写后逐写屏障覆盖写绕数据");
    }
}
