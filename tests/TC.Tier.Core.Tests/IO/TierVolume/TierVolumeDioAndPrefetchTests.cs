...
/// </summary>
public sealed class TierVolumeDioAndPrefetchTests : IDisposable
{
    private readonly string _dir = TestTempDir.Create("core-io-tv-dio");
    private readonly TierVolumeFs _fs;

    public TierVolumeDioAndPrefetchTests()
    {
        _fs = TierVolumeFs.New(TierVolumeCarrier.File(Path.Combine(_dir, "v.tier")),
            new TierVolumeFormatOptions { QuotaBytes = 64L << 20, JournalReserveBytes = 0 });
    }
