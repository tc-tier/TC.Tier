...
/// </summary>
public sealed class TierVolumeIntegrityFixesTests : IDisposable
{
    private readonly string _dir = TestTempDir.Create("core-io-tv-fixes");
    ...
    private TierVolumeFs Format(long capacity, TierVolumeFormatOptions? options = null)
    {
    ...
    {
        using var fs = Format(256L << 10, new TierVolumeFormatOptions { QuotaBytes = 256L << 10, JournalReserveBytes = 0 });
    ...
    public void MultiCarrierDispose_MemberCarrierNotGhostLocked()
    {
        ...
        using (var fs = TierVolumeFs.New(TierVolumeCarrier.File(v1), new TierVolumeFormatOptions { QuotaBytes = 8L << 20 }))
            fs.AddCarrier(TierVolumeCarrier.File(v2), 4L << 20);
        using (var fs = TierVolumeFs.Open([TierVolumeCarrier.File(v1), TierVolumeCarrier.File(v2)])) { }

        // 成员载体非独立卷（RAWC 头）——打开应报格式错误，而不是被已 Dispose 卷的幽灵登记拦成共享冲突
        var act = () =>
        {
            using var solo = TierVolumeFs.Open(TierVolumeCarrier.File(v2));
        };
    ...
    public void Format_OverMemberCarrier_ThrowsAlreadyExists()
    {
        ...
        var act = () => TierVolumeFs.New(TierVolumeCarrier.File(v2), new TierVolumeFormatOptions { QuotaBytes = 4L << 20 });
    ...
    public void RetiredBlocks_NotReusedWhileReaderProtected()
    {
        ...
        // 读者线程进入保护域（等价于 TierVolumeFileHandle.Read 的包夹）——期间收缩释放的块不得被复用
    ...
    public void HandleOps_AfterFsDispose_ThrowObjectDisposed()
    {
        ...
        IFileHandle h;
        using (var fs = TierVolumeFs.New(TierVolumeCarrier.File(path), new TierVolumeFormatOptions { QuotaBytes = 8L << 20 }))
        {
            h = fs.Open("f", RWOpts());
    ...
    public void ImageRoundtrip_PreallocatedFile_PreservesAllocationAndZeroReads()
    {
        ...
        using var src = TierVolumeFs.New(TierVolumeCarrier.File(srcPath), new TierVolumeFormatOptions { QuotaBytes = 16L << 20 });
        using var dst = TierVolumeFs.New(TierVolumeCarrier.File(dstPath), new TierVolumeFormatOptions { QuotaBytes = 16L << 20 });
    ...
    public void CrossMemberExtent_Map_DefragLandsInSingleMember()
    {
        ...
        var fs = TierVolumeFs.New(TierVolumeCarrier.File(v1),
            new TierVolumeFormatOptions { QuotaBytes = 256L << 10, JournalReserveBytes = 0 });
        _openFs.Add(fs);
        fs.AddCarrier(TierVolumeCarrier.File(v2), 256L << 10);   // 第二成员容纳物化 run
