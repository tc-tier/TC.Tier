...
public class BackgroundWorkerLoopGenericTests
{
    ...
    private sealed class MultiConsumerWorker : BackgroundWorkerLoop<int>
    {
        ...
        /// <summary>注入 scheduler（null=公共池；dedicated 形态经 <see cref="Pool"/> 注入测试私有池）。</summary>
        public MultiConsumerWorker(TaskScheduler? scheduler, int consumerCount, int target,
            bool requireAllConsumersPresent = false)
            : base(scheduler, consumerCount: consumerCount, name: "MultiConsumer")
        {
            _target = target;
    ...
    /// <summary>
    /// ★ dedicated 形态的确定性锚（2026-09-11 macos CI 实锤，2026-08-25 Windows 线程风暴同源）：
    /// 消费者是<b>协作 Task</b> 跑在调度器线程上（consumerCount ≠ 线程数），Shared 全局池 M≈宿主核数。
    /// barrier 全到场断言在 N&gt;M 时<b>结构性死锁</b>——SignalAndWait 同步阻塞占满池线程，队尾消费者
    /// 永远轮不上；N≤M 也会被同组并行测试抢线程造成饥饿。故 dedicated 测试自建 <c>ThreadCount=N</c>
    /// 私有池：N 消费者 N 线程，屏障全到场与宿主核数/并行邻居彻底解耦，InlineData 全形态在 CI 成立。
    /// 非 dedicated（公共池）不受影响——无屏障无线程数断言。
    /// <para>★ 消费者预算：调度器守卫禁 ThreadCount &gt; ProcessorCount（2 vCPU CI runner 必抛），
    /// 故 N 先按核数收缩（<see cref="Budget"/>）——屏障断言的语义核 = 扇出多线程在场 + 不丢不重，
    /// 非「恰好 InlineData 个」；本地大核自动跑全形态 4/8。</para>
    /// </summary>
    private static IsolatedTaskScheduler? Pool(bool dedicated, int consumers)
        => dedicated
            ? IsolatedTaskScheduler.Create(new IsolatedSchedulerOptions { ThreadCount = consumers, Name = $"mc-{consumers}" })
            : null;

    /// <summary>消费者核预算：≤ ProcessorCount（调度器守卫），≥ 2 保扇出语义（InlineData 最低即 2）。</summary>
    private static int Budget(int consumers)
        => Math.Clamp(consumers, 2, Math.Max(2, Environment.ProcessorCount));
    ...
    public async Task MultiConsumer_AllItemsConsumedOnce_AndActuallyConcurrent(int consumers, bool dedicated)
    {
        ...
        var budget = Budget(consumers);
        using var pool = Pool(dedicated, budget);
        var worker = new MultiConsumerWorker(pool, budget, items,
            requireAllConsumersPresent: true);
        worker.ConsumerCount.Should().Be(budget);
        worker.Start();
    ...
    public void Ctor_RejectsNonPositiveConsumerCount()
    {
        var act = static () =>
        {
            using var _ = new MultiConsumerWorker(null, consumerCount: 0, target: 1);
        };
    ...
    public void Ctor_RejectsExcessiveConsumerCount_WithClearMessage()
    {
        ...
        var act = static () =>
        {
            using var _ = new MultiConsumerWorker(null, consumerCount: BackgroundWorkerLoop.MaxConsumerCount + 1, target: 1);
        };
        act.Should().Throw<ArgumentOutOfRangeException>()
    ...
    public async Task ManyConsumers_PoolMode_StartsAndStopsCleanly()
    {
        ...
        using var worker = new MultiConsumerWorker(null, consumerCount: 16, target: 1);
        worker.Start();
        worker.Enqueue(1);
