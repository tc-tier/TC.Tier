...
public static class SyncAsyncBridge
{
        ...
        SyncBridgeOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        // ★ work 在桥池线程启动（不可在调用线程预启动/短路——ValueTask 启动后无法换上下文，
        //   work 内部无 CA 续体（Task.Yield 等）的调度回落脚=启动点 TaskScheduler：调用线程启动
        //   =续体落公共池——"推进不依赖公共池"的隔离语义破坏（池饿死回归实锤）。同步完成场景的
        //   就地零跳推进是 AsyncPump 的领地——桥与泵的分工）。
        ...
        // ★ wrapper 对 work 的 await **不** ConfigureAwait(false)：捕获 TaskScheduler.Current（=桥池）
        //   ——wrapper 收尾段（ReportSucceeded/ReportCanceled/ReportFailed）回流桥池私有线程，
        //   "推进不依赖公共池"兑现（work 内部自行 ConfigureAwait(false) 的段落在线程池——那是
        //   work 的选择；桥自身收尾必回桥池）。
        // ★ 受控丢弃（TCSG138 豁免——治理内核唯一丢弃点，对齐 TaskSink 内部丢弃点先例）：
        //   wrapper 异常全量收口进 op（下述 try/catch 全捕获，Task 永不 fault），观察面即 op（Wait/Describe/LeakDetect）；
        //   改经 TaskSink 反而引入二次派发——破坏"work 必在桥池线程启动"的隔离语义（见上注）。
#pragma warning disable TCSG138 // 设计必需：治理内核自带观察面——异常不逃（全捕获进 op），无人观察的仅是永不 fault 的 wrapper 壳
        _ = Task.Factory.StartNew(async () =>
        {
            SCurrentBridgeScheduler.Value = scheduler;   // 随 work 的 await 流动（再入检测的判据）——work 启动前设置
            try
            {
