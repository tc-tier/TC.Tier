...
public sealed class IsolatedTaskScheduler : TaskScheduler, IDisposable
{
    ...
    /// </summary>
    /// <param name="options">调度器选项（线程数/名称/日志等）；null = 全默认。</param>
    /// <returns>新创建的独立调度器实例（已注册 <see cref="InstanceTracker"/>，随 <c>Resources</c> 释放）。</returns>
    ...
    /// <returns>恒返回 false——本调度器不支持出队。</returns>
    ...
    /// <returns>恒返回 false——本调度器不支持内联执行。</returns>
