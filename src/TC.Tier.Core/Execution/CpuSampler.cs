...
public sealed class CpuSampler : BackgroundWorkerLoop
{
    ...
    private readonly TimeProvider _clock;        // 时钟供给源（时钟缝 件一——采样周期/墙钟差统一）
    ...
    /// <param name="clock">时间供给源（采样节拍；null = 系统时钟）。</param>
    ...
    {
        _clock = clock ?? TimeProvider.System;   // 时钟缝 件一（P1 落点）
    ...
    /// <returns>任务在采样循环退出（ct 取消或 timer 不再 tick）后完成；恒返回 false——本实现内含完整
    /// while 采样循环，一次调用即承担整个 worker 生命周期（true=继续下一周期的基类模板契约不适用）。</returns>
    protected override async ValueTask<bool> RunOneCycleAsync(CancellationToken ct)
    {
        _process ??= Process.GetCurrentProcess();
        // 基准
        _lastCpuTime = _process.TotalProcessorTime;
        _lastSampleTicks = _clock.GetMsTimestamp();

        while (!ct.IsCancellationRequested)
        {
            // ★ 时钟缝 件一：周期等待经供给源（假钟下由快进确定性驱动；System 直通 Task.Delay）
            await _clock.Delay(_sampleInterval, ct).ConfigureAwait(false);
    ...
    private void Sample()
    {
        ...
        var wallMs = _clock.GetMsTimestamp() - _lastSampleTicks;
        if (wallMs <= 0)
