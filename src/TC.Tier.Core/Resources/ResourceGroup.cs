...
namespace TC.Tier.Core.Resources;
...
public sealed class ResourceGroup : IDisposable, IAsyncDisposable
{
    ...
    /// <param name="resource">要添加的资源——须实现 <see cref="IDisposable"/> 或 <see cref="IAsyncDisposable"/>，非空。</param>
    /// <param name="name">注册名；缺省用 <c>resource.GetType().Name</c>。</param>
    /// <param name="ownership">所有权模式（Owned=我释放 / Referenced=外部管只跟踪）。默认 <see cref="ResourceOwnership.Owned"/>。</param>
    /// <returns>true 表示添加成功；false 表示重名、已释放、或资源不实现 IDisposable/IAsyncDisposable（不抛异常）。</returns>
    ...
    /// <summary>按名移除登记（调用方自管生命周期的资源退出——如"删组后重建同名"场景：
    /// 旧登记不摘除，重建组的新引擎将无法注册，实例释放时成为漏网之鱼）。线程安全。</summary>
    /// <param name="name">注册名。</param>
    /// <returns>true 表示已摘除；false 表示名不存在（幂等）。</returns>
    ...
    /// <inheritdoc/>
    /// <returns>任务在快照内全部资源释放（逆序，仅 Owned）完成后结束；个别资源释放失败则任务以
    /// <see cref="AggregateException"/> 完成（异常聚合展平，不中断其余资源的释放）。</returns>
    ...
    private static List<Exception>? DisposeCoreSync(List<object> snapshot)
    {
        ...
        foreach (var r in snapshot)
        {
            try
            {
                ...
                {
#pragma warning disable TCSG137 // 设计必需：Dispose 必须同步完成（IDisposable 契约无 async 形态）
