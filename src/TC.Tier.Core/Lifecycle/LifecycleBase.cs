...
namespace TC.Tier.Core.Lifecycle;
    ...
    where THints : struct
{
    ...
    /// <param name="ct">取消令牌——等待期间被取消则任务以 <see cref="OperationCanceledException"/> 完成。默认 <c>default</c>。</param>
    /// <returns>已就绪时为 <see cref="Task.CompletedTask"/>；否则为恢复 task 的 <c>WaitAsync</c> 包装——恢复成功正常完成，
    /// 恢复失败以相同异常完成，无恢复 task（终态）时为 <see cref="Task.CompletedTask"/>。</returns>
    ...
    /// </summary>
    /// <param name="hints">恢复 hints（各持有者自定义的启动参数 struct，转发给恢复算法）。默认 <c>default</c>。</param>
    public void Initialize(THints hints = default)
    {
        ...
        {
            // ★ 取消/失败重试竞窗守卫（#419 实锤）：RecoverAsync 置 Failed（RecoveryBase catch）与
            //   外层 OCE catch 写 _initialized=0 非原子——2vCPU 慢机上窗口秒级，重试 Initialize 被
            //   CAS 静默拒绝 = 恢复永不重启（IsReady 永假）。Failed + 旧 task 存在 = 收尾中——等其
            //   彻底结束后重取 CAS；活跃恢复中（Recovering）/已就绪/无 task = 真幂等拒绝，静默返回。
            var state = RecoveryState;
            var prior = Volatile.Read(ref _recoverTask);
            if (state.Phase != RecoveryPhase.Failed || prior is null)
                return;
            try
            {
#pragma warning disable TCSG137 // 设计必需：Initialize 同步重试守卫——等旧 task 收尾（终态有界，低频路径）
                prior.Wait();
#pragma warning restore TCSG137
            ...
            if (Interlocked.CompareExchange(ref _initialized, 1, 0) != 0) return;   // 仍占用（执行期失败 _initialized 保持 1——实例不可修复契约）
        }
        ...
        if (priorTask is not null)
        {
            try
            {
#pragma warning disable TCSG137 // 设计必需：Initialize 同步重试守卫——等旧 task 收尾（低频路径）
                priorTask.Wait();
#pragma warning restore TCSG137
            }
            catch
    ...
    /// </summary>
    /// <param name="worker">要托管的后台 worker（<see cref="BackgroundWorkerLoop"/> 派生实例，业务逻辑已填好），非空。</param>
    ...
    /// </summary>
    /// <returns>任务在取消恢复与清理、子类异步钩子、资源组异步释放全部完成后完成；重复 Dispose 幂等立即完成。</returns>
    ...
    /// <param name="disposing">true=用户经 <see cref="DisposeAsync"/> 调用（可清理托管资源）；false=终结器路径（不可触托管）。
    ///   本基类终结器不做资源清理（仅告警），当前恒以 true 调用，供子类判断。</param>
    /// <returns>默认 <see cref="ValueTask.CompletedTask"/>；子类实现时任务在额外清理完成后完成。</returns>
