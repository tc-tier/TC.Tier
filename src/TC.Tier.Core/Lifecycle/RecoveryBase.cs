    ...
    where THints : struct
{
    ...
    /// </summary>
    /// <param name="hints">恢复 hints（由各持有者自定义的 struct，传给 <see cref="OnRecoveryCoreAsync"/>）。</param>
    /// <param name="ct">取消令牌（传递给依赖等待与恢复核心；取消并入 Failed 语义，见上）。默认 <c>default</c>。</param>
    /// <returns>已 Ready 时立即完成（幂等 no-op）；否则任务在编排五步走完后完成，恢复失败/取消则以原始异常
    /// 完成（状态置 <see cref="RecoveryPhase.Failed"/>，闸门回 Unrecovered）。</returns>
    ...
    /// </summary>
    /// <param name="ct">取消令牌（等待期间取消则抛 <see cref="OperationCanceledException"/>，并入 Failed 语义）。</param>
    /// <returns>默认 <see cref="ValueTask.CompletedTask"/>；子类实现时任务在全部依赖就绪后完成。</returns>
    ...
    /// </summary>
    /// <param name="hints">恢复 hints（Initialize 传入的启动参数 struct）。</param>
    /// <param name="ct">取消令牌——在 IO checkpoint 处 <c>ct.ThrowIfCancellationRequested()</c> 检查。</param>
    /// <returns>任务在恢复核心算法完成后完成；失败/取消时以异常完成（模板统一置 Failed 并重抛）。</returns>
