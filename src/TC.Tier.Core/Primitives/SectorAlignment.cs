...
/// <para>★ <paramref name="alignment"/> 必须是 2 的幂且为正——底层位运算 <c>(value + mask) &amp; ~mask</c>
///   仅对 2 幂成立：传 0 会静默返回 0、传非 2 幂/负数会静默产生错位对齐值，极难排查
///   （IO 对齐错误会引发 O_DIRECT 失败、数据错位等连锁问题）。</para>
/// <para>★ 入口用 <see cref="BitOperations.IsPow2"/> 校验（单条 CPU intrinsic，纳秒级，分支预测下几乎零开销）+
///   <see cref="Utilities.ThrowHelper"/>（<c>[DoesNotReturn]</c> + <c>NoInlining</c>）隔离冷 throw 路径，
///   热路径内联不受影响。</para>
/// </summary>
public static class SectorAlignment
{
    /// <summary>校验 alignment 为正且 2 的幂。<see cref="Validate(int)"/> 标 AggressiveInlining →
    /// <see cref="BitOperations.IsPow2"/> 检查内联进调用方；throw 路径经 ThrowHelper 隔离，不阻碍调用方内联。</summary>
