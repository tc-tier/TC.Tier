...
public sealed partial class RawFileSystem
{
    ...
    /// <summary>
    /// 预取跟随（Advise(Sequential) 读后调用——RawFileHandle.Read）：逻辑终点 <paramref name="logicalPos"/>
    ...
    /// 无页机制开销——排干前置由调用方保证（RawFileHandle.Read 读前一次排干）。</summary>
    ...
    /// 主路径（RawFileHandle.Write/Append）走 <see cref="WriteDataPlanned"/>（三段式锁外化）。
    /// </summary>
    internal void WriteData(Entry e, long offset, ReadOnlySpan<byte> source, bool direct = false)
    {
        ...
        while (done < source.Length)
        {
            var pos = offset + done;
            Extent hit;
            if (FindExtent(e.Extents, pos) is { } found)
            {
                hit = found;
            }
            else
    ...
    /// <summary>
    /// ★ 文件数据写（写计划协议——CORE-02 主路径）：三阶段锁外化。
    /// ① 规划段（WriteGate + MetadataLock）：快照 → 区间决策/分配/日志发射 → 构造工作列表（不发布）+ 零基需求；
    /// ② 数据段（WriteGate + 锁外 + epoch 钉块）：载体写/页操作/零基——锁内只留微秒级元数据；
    /// ③ 提交段（WriteGate + MetadataLock）：发布工作列表 + unwritten 转换 + 推长度 + 游标 + 脏标。
    /// ★ 不变量：发布在数据段之后（读者见新区间 ⟹ 数据已在载体/脏页）；崩溃窗口与 v1 同构
    /// （分配/记录在规划段、数据写后屏障——记录未过屏障 = 位图恢复未分配，无泄漏）。
    /// ★ 同文件写经 <see cref="Entry.WriteGate"/> 串行；不同文件完全并行；锁序 WriteGate → MetadataLock 单向。
    /// </summary>
    internal void WriteDataPlanned(Entry e, long offset, ReadOnlySpan<byte> source, bool direct = false)
    {
        if (source.IsEmpty) return;
        using (SpinLockScope.Enter(ref e.WriteGate))   // 同文件写串行（SpinLock 无竞争 ~5ns）
        {
            var bs = (long)_pageSize;
            ...
            // ── ① 规划段（MetadataLock 内——微秒级：纯内存 + 分配 + 记录发射）──
            lock (MetadataLock)
            {
                // working 直接引用在档列表（FileGate 保证同文件单写者——本写全程独占；CoW 由
                // EnsureExtentCovering 按需做——追加主路径零复制）；提交段才整体交换发布（I4）
                working = e.Extents;
                ...
                while (done < source.Length)
                {
                    var pos = offset + done;
                    Extent hit;
                    if (FindExtent(working, pos) is { } found)
                    {
                        hit = found;
                    }
                    else
                    {
                        ...
                        working = EnsureExtentCovering(e, working, RoundDown(pos, bs), coverEnd - RoundDown(pos, bs), writeEnd, direct, pos, zeroOps);
            ...
            // ── ② 数据段（锁外——载体 IO/页操作/零基，毫秒级不阻塞任何元数据路径）──
            // ★ 顺序与 v1 一致：零基先行（未覆盖残段归零）、数据后写（同块被写覆盖——v1 规划段零基语义）
            // ★ 钉块：per-Entry 在途写者计数（删除/截断/打洞锁内自旋等归零——见 FreePhysical/删除路径）——
            // 替代 epoch Resume/Suspend（每写省 ~100ns；写者数据段锁外不碰锁，持锁方等待无死锁环）
            Volatile.Write(ref e.WritersInFlight, 1);   // 写者独占转换（WriteGate 串行——0/1 单写者）
            try
            {
            ...
            {
                // ★ 先清写者标记（删除的等待判据）再补发布——防"删除持锁自旋 vs 本线程补发布等锁"互等
                Volatile.Write(ref e.WritersInFlight, 0);
                // ★ v1 失败语义复刻：区间补发布（含已写部分数据）——否则规划段已分配块无主泄漏
                // （v1 的 EnsureExtentCovering 在失败前已发布区间；此处规划段未发布——失败时补上，块随区间有主）。
                // 长度不推（v1 同：失败时 grew 代码未执行）；异常照抛。
                if (metaChanged)
                    lock (MetadataLock)
                    {
                        e.Extents = working;
            ...
            if (metaChanged || grew || anyUnwritten)
            {
                lock (MetadataLock)
                {
                    ...
                    e.Extents = working;   // 整体交换发布（I1：锁外读者持旧列表——完整旧态或完整新态）
                    if (grew)
                    {
                        e.LogicalLength = Math.Max(e.LogicalLength, offset + source.Length);
                        // D10：位置写增长 EOF——追加预留权威单调跟进
                        if (_appendCursors.TryGetValue(e.Path, out var cursor))
                        {
                            var newLen = e.LogicalLength;
                            while (true)
            ...
            TouchModified(e);   // mtime 锁外更新（8B 原子写 + bool 原子——v1 每写语义；同文件 WriteGate 串行独占）
        }
    }
