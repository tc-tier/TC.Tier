...
/// <summary>
/// Raw 文件句柄——<see cref="IFileHandle"/> 的 Raw 实现（数据面经 fs 统一页管理，§3.4）。
...
/// </summary>
internal sealed class RawFileHandle : IFileHandle, IPoolAttachable
{
    private readonly RawFileSystem _fs;
    private readonly RawFileSystem.Entry _entry;
    ...
    internal RawFileHandle(RawFileSystem fs, RawFileSystem.Entry entry, FileOpenOptions options)
    {
        _fs = fs;
        _entry = entry;
        _path = entry.Path;
    ...
    /// <summary>读（RM-12 锁外快照）：锁内捕获不可变视图（区间列表 CoW + 逻辑长度）→ 释放锁 →
    /// 页缓存/载体读全程无锁（多读者并行——写者以快照交换推进，读者见完整旧态或完整新态）。
    /// D1b：epoch 保护域包夹全程——回收延迟至本读者退出（快照引用的块不可被复用）。
    /// 流式（性能债 6 收尾）：Advise(Sequential) 缓冲档读 = 纯流式模式——读前一次排干自管脏页
    /// （可见性收口）→ 全程载体直读不驻留（无页机制开销、无预读交互），OS 缓存自然服务重复扫描。</summary>
    public int Read(long offset, Span<byte> destination)
    {
        ...
        try
        {
            RawFileSystem.DataSnapshot snap;
            lock (_fs.MetadataLock)
    ...
    /// <inheritdoc/>
    /// <remarks>Raw 区间三态的真实投射（§3.2）——unwritten 标注保真（采集管线 D4 依赖）。</remarks>
    public IReadOnlyCollection<(long Start, long End, bool Unwritten)> EnumerateAllocatedRangesDetailed()
    {
        ...
        lock (_fs.MetadataLock)
            return _entry.Extents
                .Select(x => (x.LogicalStart, x.LogicalEnd, x.State == RawFileSystem.ExtentState.Unwritten))
                .ToList();
    }
    ...
    public long CopyRange(IFileHandle destination, long sourceOffset, long destinationOffset, long length)
    {
        ...
        if (destination is not RawFileHandle dest || !ReferenceEquals(dest._fs, _fs))
            throw new ArgumentException($"CopyRange requires a Raw handle destination on the same volume.", nameof(destination));
    ...
    public IMappedSection Map(long offset, long length, AccessMode access)
    {
        ...
        lock (_fs.MetadataLock)
        {
            var covering = _entry.Extents is [{ State: RawFileSystem.ExtentState.Written, LogicalStart: 0 } x]
                           && x.LogicalEnd >= _entry.LogicalLength
                ? _entry.Extents[0]
                : default(RawFileSystem.Extent?);
