...
public sealed partial class RawFileSystem
{
    ...
    /// 日志模式（raw-journal §5）：兼作检查点——CkptLsn 前进 + 区代数复位 + head=0 随翻转原子生效。</summary>
    internal void CommitMetadata()
    {
        if (_readOnly)
    ...
    /// <summary>★位图=可达集 对账：重算可达块集，重写内存位图并落盘（dirty 恢复路径）。</summary>
    private void ReconcileBitmapToReachable()
    {
        ...
        {
            var shouldUse = reachable.Contains(b);
