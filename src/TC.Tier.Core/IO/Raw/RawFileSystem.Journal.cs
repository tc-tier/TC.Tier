...
public sealed partial class RawFileSystem
{
    ...
    internal void JournalCommit(bool holdLock = true)
    {
        ...
        while (true)
        {
            ...
            lock (MetadataLock)
            {
                ...
                {
                    emptyRecords = true;
                    dataOnly = Volatile.Read(ref _dirtyBytes) > 0;
                }
            ...
            lock (_journalGate)
            {
                ...
                FlushCarrier();   // 单屏障——数据页 + 日志记录同一 fsync 覆盖
    ...
    private void JournalReplay()
    {
        ...
        while (offset < areaLen)
        {
            ...
            if (lsn > _sb.JournalCkptLsn && type != JournalRecordType.Pad)
            {
                apply.Add((lsn, type, body));
        ...
        _committedLsn = _lsn;
        _journalHead = offset >= areaLen ? 0 : offset;   // 追加起点 = 有效前缀末尾
    }
