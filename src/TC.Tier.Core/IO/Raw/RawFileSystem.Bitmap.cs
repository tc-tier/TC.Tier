...
public sealed partial class RawFileSystem
{
    ...
    private ulong AllocateBlocks(uint count, string purpose, ulong? maxBlock = null, ulong? minBlock = null)
    {
        ...
        var found = start + count <= total
            ? ScanFree(start, count, total)   // 提示点起扫
            : null;
        found ??= ScanFree(min, count, total);   // 界内全量重扫（碎片化兜底）
        if (found is { } s)
        {
            MarkBlocks(s, count, used: true);
            _allocHint = (s + count) % _sb.CapacityBlocks;
            return s;
        }
    ...
    private bool TryExpandCapacity(uint neededBlocks, string purpose)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                MarkBlocks(oldBitmapStart, (uint)oldBitmapBlocks, used: false);
    ...
    /// <summary>设备载体 TRIM（BLKDISCARD——RM-05）：块真正回收时对设备区间下发丢弃提示。
    /// 优化非正确性（B1 零基纪律独立成立——TRIM 失败仅损失 SSD 寿命优化）；advisory 失败静默。
    /// 仅 Linux + 设备成员；字节区间按成员内本地偏移（设备布局自偏移 0 起）。</summary>
    private unsafe void TrimDeviceBlocks(ulong block, uint count)
    {
        if (!OperatingSystem.IsLinux() || count == 0) return;
        var range = stackalloc long[2];   // CA2014：stackalloc 不入循环
        foreach (var m in _members)
        {
            if (m.IsMissing || !m.Carrier.IsDevice) continue;
            ...
            try
            {
                ...
                try
                {
                    range[0] = (long)((s - m.BaseBlock) * (ulong)_pageSize);
                    range[1] = (long)(e - s) * _pageSize;
