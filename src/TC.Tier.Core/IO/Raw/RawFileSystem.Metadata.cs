...
public sealed partial class RawFileSystem
{
    ...
    internal sealed class Entry
    {
        ...
        /// <summary>数据段在途写者标记（0/1——同文件写经 WriteGate 串行，写者独占转换）：
        /// 删除/截断/打洞等"锁内释放块"路径的等待判据：持 MetadataLock 自旋等归零（写者数据段
        /// 锁外不碰锁——无死锁环；异常路径先清标记再补发布）。
        /// ★ 替代数据段 epoch 钉块（每写省 Resume/Suspend ~100ns）+ Interlocked 降 volatile（省栅栏）。</summary>
    ...
    private void LoadMetadata(byte[] image)
    {
        ...
        if (r.ReadUInt16() != RawLayoutVersion)
    ...
    // 只还 _safeBatch 之内的批次。读者（RawFileHandle.Read）Resume/Suspend 包夹快照捕获与读取；
    ...
    private void TryFreeRetiredLocked()
    {
        ...
        _retiredBlocks.RemoveAll(t =>
        {
            ...
            TrimDeviceBlocks(t.Block, t.Count);   // RM-05：真正回收点下发 TRIM（epoch 读者已退出）
            return true;
        });
        EnsureRetireBumpLocked();
    }
