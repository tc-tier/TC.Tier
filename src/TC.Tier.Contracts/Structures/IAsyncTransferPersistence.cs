...
/// <summary>
/// 异步流式读写传输持久化通道——结构层通用的三段式流式读写契约（任何结构、任何后端实现）。
/// </summary>
public interface IAsyncTransferPersistence : IAsyncDisposable
{
    /// <summary>
    ...
    ValueTask<bool> TryOpenWriteAsync(out IAsyncTransferWriter? writer, int maxTransferBytes = DefaultMaxTransferBytes);
    ...
    ValueTask<bool> TryOpenReadAsync(out IAsyncTransferReader? reader, int maxTransferBytes = DefaultMaxTransferBytes);
}