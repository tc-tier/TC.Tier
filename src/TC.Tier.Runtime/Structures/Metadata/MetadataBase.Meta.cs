...
public abstract partial class MetadataBase
{
        ...
        : IMetaLayout<MetadataMetaHeader, MetadataMetaPayload>
    {
        ...
        /// <returns>解析出的 MetadataMetaHeader。</returns>
        ...
        /// <returns>解析出的 MetadataMetaPayload。</returns>
        ...
        /// <returns>默认 MetadataMetaHeader（规范字段常量填好，PayloadLength/PaddingLength 为零）。</returns>
    ...
    private protected sealed class MetaHost(MetadataBase owner) : IMetaTransport
    {
        ...
        /// <summary>正向扫版本链找最后一条 IS_META record 并返回其 payload（meta block）。</summary>
        /// <returns>meta block 字节视图（视图有效至本传输下一次调用）；无 meta record 时为空 Span。</returns>
        ...
        /// <summary>读回最后一条 meta block（异步对等版——实质同步扫描）。</summary>
        /// <param name="ct">取消令牌（当前实现不检查取消）。</param>
        /// <returns>meta block 字节视图（视图有效至本传输下一次调用）；无 meta record 时为空 Memory。</returns>
        ...
        /// <param name="block">完整 meta 块字节（写为 record payload；不推进版本链水位）。</param>
        ...
        /// <param name="ct">取消令牌（当前实现不检查取消）。</param>
        /// <returns>表示异步写入完成的任务；完成后 meta block 已作为 IS_META record 落盘（含引擎 flush）。</returns>
