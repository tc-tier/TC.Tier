...
/// <summary>
/// SortedIndexBase 恢复 partial（DefaultSortedIndexBaseRecovery——RecoveryBase 模板派生，不裸写 IRecovery：
/// 底层生命周期骨架是压测背书的信任边界，结构层只填钩子）。
/// <para>★ 自建恢复（设计稿 §4）：索引=派生数据，重建数据面 = 真相源 record 流——
///   建空结构后拉 <see cref="IKeyResolver{TKey}"/>.ScanAsync 窗口流逐条 Insert 增量插节点
///   （复用公开 Insert：比较路由/同 key 覆盖语义原样生效，流序折叠 = 最新写胜出）。</para>
/// <para>★ 三级回退（统一生命周期——主存储载入归 Persistence.cs 中间级）：hints（调用方最强知识）
///   → 主存储最新完整帧（载入物化 + 重放 (W, End]）→ 建空结构 + Ring 全量重放兜底。</para>
/// </summary>
    ...
    where TKey : unmanaged, IEquatable<TKey>
{
    ...
    private protected class DefaultSortedIndexBaseRecovery(SortedIndexBase<TKey> owner) : RecoveryBase<SortedIndexRecoveryHints>
    {
        ...
        protected override async ValueTask OnRecoveryCoreAsync(SortedIndexRecoveryHints hints, CancellationToken ct)
        {
            ...
            // ★ 锚点槽预留（引擎已就绪、节点未分配——首开=MinAddress，重开=原位）
            owner.EnsureAnchorReserved();
            ...
            {
                if (owner.KeyResolver is null)
                    throw new InvalidOperationException(
                        "比较族重放窗口需要 IKeyResolver（恢复数据面）——构造期未注入，无法自建恢复");
