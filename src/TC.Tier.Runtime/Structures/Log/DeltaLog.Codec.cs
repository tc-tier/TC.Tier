...
/// <summary>
/// DeltaLog 编解码 partial——嵌套 Codec（<see cref="ILogCodec"/> 实现）：记录头写入与解析。
/// <para>★ Header 布局见 <see cref="DeltaLogHeader"/>；CRC 由 <see cref="RecordCodec"/> 统一处理。</para>
/// </summary>
public sealed partial class DeltaLog
{
    ...
    private sealed class Codec : ILogCodec
    {
        ...
        /// <summary>
        /// 写入单条 delta 记录头：填规范字段（Flags 叠加 IS_META 位）并计算 CRC32C
        /// （覆盖 header 除 Crc 自身 + payload + padding）写入头内 Crc 字段。
        /// </summary>
        /// <param name="dest">目标缓冲区；长度须 ≥ <see cref="Codec.HeaderSize"/>（18 字节）且按 4 字节对齐（<see cref="Codec.Alignment"/>）。</param>
        ...
        /// <param name="isMeta">true = 元数据记录（Flags 置 FLAG_ENTRY_IS_META）；false = 普通增量记录。</param>
        ...
        /// 尝试从源缓冲区解析 delta 记录头。
        /// </summary>
        /// <param name="source">源缓冲区；长度不足 HeaderSize（18 字节）直接失败。</param>
        /// <param name="payloadLength">输出 payload 字节数（不含 header/padding）；解析失败时为 0。</param>
        /// <param name="paddingLength">输出对齐补零字节数；仅成功时有效。</param>
        /// <param name="isMeta">输出 true = 元数据记录、false = 普通增量记录；解析失败时为 false。</param>
        /// <param name="verifyCrc">true = 额外做 CRC32C 校验（覆盖 header+payload+padding），默认 false。</param>
