...
/// <summary>
/// TierVolume 页缓存并发正确性探针（CORE-01/RM-12 竞态封闭回归验证）——预算压力场景：
...
/// </summary>
internal static class TierVolumePageCacheProbe
{
    ...
    public static int Run(string[] args)
    {
        ...
        Console.WriteLine($"===== TierVolumePageCacheProbe（{seconds}s）——预算 {PageBudgetBytes >> 20}MB / 写者 {PerWriterBytes >> 20}MB / {FileCount} 文件 =====");
        ...
        try
        {
            using var fs = TierVolumeFs.New(TierVolumeCarrier.File(Path.Combine(dir, "volume.tier")),
                new TierVolumeFormatOptions { BlockSize = 4096, QuotaBytes = 256L << 20 });
